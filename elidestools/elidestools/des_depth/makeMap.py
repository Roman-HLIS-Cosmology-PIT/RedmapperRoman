...
class MakeMap(object):
    ...
    def run(self):
        ...
        for b, band in enumerate(self.bands):
            altBand = self.altBands[0]
            if (band == altBand):
                altBand = self.altBands[1]
            ...
            mPred = np.concatenate([rf.predict(xMap[i:i+1_000_000]) for i in range(0, len(xMap), 1_000_000)])
